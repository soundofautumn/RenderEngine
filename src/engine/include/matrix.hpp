...
struct Matrix {
    ...
    inline Matrix(const Matrix &other) { memcpy(m, other.m, sizeof(m)); }

    inline Matrix(std::initializer_list<std::initializer_list<T>> list) {
        size_t i = 0;
        for (auto &row : list) {
            size_t j = 0;
            for (auto &col : row) {
    ...
    inline Vector<COL, T> row(size_t r) const {
        assert(r < ROW);
        Vector<COL, T> v;
    ...
    inline Vector<ROW, T> col(size_t c) const {
        assert(c < COL);
        Vector<ROW, T> v;
    ...
    inline void set_row(size_t r, const Vector<COL, T> &v) {
        assert(r < ROW);
        for (size_t i = 0; i < COL; i++) {
            m[r][i] = v[i];
        }
    ...
    inline void set_col(size_t c, const Vector<ROW, T> &v) {
        assert(c < COL);
        for (size_t i = 0; i < ROW; i++) {
            m[i][c] = v[i];
        }
...
inline Matrix<ROW, NEW_COL, T> operator*(
