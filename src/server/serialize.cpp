...
Primitive deserialize_primitive(const boost::json::object &primitive) {
    if (primitive.contains("Line")) {
        ...
        return RenderCore::Line{.p1 = deserialize_point(p1),
        ...
        return RenderCore::CircleUseCenterRadius{.center = deserialize_point(center),
        ...
        return RenderCore::ArcUseCenterRadiusAngle{.center = deserialize_point(center),
            .radius = static_cast<int>(radius),
...
boost::json::object serialize_primitive(const Primitive &primitive) {
    if (std::holds_alternative<RenderCore::Line>(primitive)) {
        ...
        return {{"Line", {{"p1", {{"x", line.p1.x}, {"y", line.p1.y}}},
        ...
        if (std::holds_alternative<ArcUseCenterRadiusAngle>(arc)) {
            ...
            return {{"Arc",
                {{"center", {{"x", arc_.center.x}, {"y", arc_.center.y}}, {"radius", arc_.radius},
                    {"start_angle", arc_.start_angle}, {"end_angle", arc_.end_angle},
